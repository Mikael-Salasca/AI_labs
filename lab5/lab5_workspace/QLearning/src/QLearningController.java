...
public class QLearningController extends Controller {
	...
	void performAction(int action) {
		...
		/* Fire zeh rockets! */
		/* Remember to change NUM_ACTIONS constant to reflect the number of actions (including 0, no action) */		
	...
	public void tick(int currentTime) {
		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				Double prev_qval = Qtable.get(prev_stateaction);
				if (prev_qval == null) {
					Qtable.put(prev_stateaction, 0.0);
				} else {
					double a      = alpha(Ntable.get(prev_stateaction));
					double r      = previous_reward;
					double maxQsa = getMaxActionQValue(new_state);
					double qval   = prev_qval + a * (r + GAMMA_DISCOUNT_FACTOR * maxQsa - prev_qval);
